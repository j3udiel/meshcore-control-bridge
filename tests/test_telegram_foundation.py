...
from meshcore_control.telegram.client import TelegramBotApiClient, TelegramRateLimitError
...
async def test_duplicate_update_is_ignored_after_restart(tmp_path: Path) -> None:
    ...
    first = service.process_update(_message(20))
    ...
    duplicate = restarted_service.process_update(_message(20))
...
def test_ignored_update_types(tmp_path: Path, update: dict[str, Any], reason: str) -> None:
    service = TelegramFoundationService(
        config=_config(),
        client=FakeTelegramClient(),
        store=_store(tmp_path),
    ...
    decision = service.process_update(update)
...
def test_sqlite_and_logs_do_not_store_private_values(tmp_path: Path, caplog) -> None:
    token_file = tmp_path / "telegram.bot_token"
    load_or_import_token(token_import=VALID_TOKEN, token_file=str(token_file))
    store = _store(tmp_path)
    service = TelegramFoundationService(
    ...
    with caplog.at_level("INFO"):
        service.process_update(_message(50, text="sensitive body"))
...
def test_audit_uses_hmac_references(tmp_path: Path) -> None:
    store = _store(tmp_path)
    service = TelegramFoundationService(
        config=_config(),
        client=FakeTelegramClient(),
    ...
    service.process_update(_message(60))
