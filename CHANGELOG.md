...
- Runtime overrides are held only in memory and reset on App restart.
- `!bridge` remains readonly when called without arguments.
- Admin changes affect forwarding decisions for new messages only; they do not
  close transports, cancel polling, or modify pending bridge records.
- Health snapshots and Home Assistant health events report effective forwarding
  state plus redacted `runtime_overrides`.

### Security

- Override subcommands require the `admin` role.
- The authorized Telegram chat/user remains `readonly` by default.
- Override audit events store only allow-listed structured metadata.
- No tokens, raw IDs, pubkeys, message text, command arguments, paths, or
  correlation IDs are exposed.

### Not Included

- Persistent remote configuration.
- Home Assistant service calls for administration.
- Write commands.
- USB release work.
...
- Bridge status.
- Version and uptime.
- MeshCore and Telegram transport state.
- Last activity in both forwarding directions.
- Last safe error.
- Success and failure counters.
- Command counter.
- Audit and Telegram database health.

### Security / Behavior

- Health data remains redacted.
- No tokens, chat IDs, user IDs, sender IDs, pubkeys, message IDs, message text,
  entity IDs, paths, or correlation IDs are exposed.
- No native Home Assistant internals are used.
- No direct Home Assistant database writes.
- No forwarding, authorization, or command behavior changes.
...
- Telegram receives a detailed activity summary.
- MeshCore receives a compact LoRa-friendly summary.
- Data is read from the in-memory `BridgeHealthState`.
- No SQLite query is performed for the command.
- Activity counters and timestamps reset when the App restarts.

### Security

- `!last` excludes tokens, raw IDs, pubkeys, message IDs, message text, command
  contents, entity IDs, paths, and correlation IDs.
- Last-error output remains restricted to sanitized safe reasons.
- No command bridging or write commands are introduced.

### Compatibility

- Existing forwarding, echo prevention, and authorization remain unchanged.
- `/data/health.json` and Home Assistant health events remain unchanged.
- No persistence is added in this release.

### Not Included

- Persistent counters.
- Administrative bridge controls.
- MQTT or HACS integration.
- USB release work.
